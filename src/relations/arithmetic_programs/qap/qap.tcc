...
bool qap_instance_evaluation<FieldT>::is_satisfied(const qap_witness<FieldT> &witness) const
{
    ...
    ans_A = ans_A + naive_exp<FieldT, FieldT>(this->At.begin()+1, this->At.begin()+1+this->num_vars,
                                              witness.coefficients_for_ABCs.begin(), witness.coefficients_for_ABCs.begin()+this->num_vars);
    ans_B = ans_B + naive_exp<FieldT, FieldT>(this->Bt.begin()+1, this->Bt.begin()+1+this->num_vars,
                                              witness.coefficients_for_ABCs.begin(), witness.coefficients_for_ABCs.begin()+this->num_vars);
    ans_C = ans_C + naive_exp<FieldT, FieldT>(this->Ct.begin()+1, this->Ct.begin()+1+this->num_vars,
                                              witness.coefficients_for_ABCs.begin(), witness.coefficients_for_ABCs.begin()+this->num_vars);
    ans_H = ans_H + naive_exp<FieldT, FieldT>(this->Ht.begin(), this->Ht.begin()+this->degree+1,
